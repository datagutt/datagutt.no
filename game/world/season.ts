// Seasons (docs/game/PLAN.md M3.9). Outdoor maps are generated in summer; for every
...
/** The season on `date` in Norway: Dec–Feb winter, Mar–May spring, Jun–Aug summer, Sep–Nov autumn. */
export function seasonOn(date: Date, timeZone = "Europe/Oslo"): Season {
...
/** Today's season in Norway, or the one asked for with `?debug&season=<name>`. */
export function resolveSeason(search: string, now = new Date()): Season {
	const params = new URLSearchParams(search);
	const asked = params.has("debug") ? params.get("season") : null;
	return isSeason(asked) ? asked : seasonOn(now);
}
