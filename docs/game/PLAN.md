...
Task format: `- [ ] **M1.3** Title: what to do. *Done when* the observable check.`
...
- [ ] **M1.3** Asset pipeline v1 `scripts/assets/build.mjs`: read the manifest
      ...
      modes produce loadable atlases.
- [ ] **M1.4** Tiled loader: load `.tmj` maps, tile layers, object layers (`door`, `npc`,
...
- [ ] **M1.5** Grid movement: 4-direction, tile-locked, walk and run speeds, facing, input
      buffering so turning feels tight (Celeste standard: no dropped inputs). *Done when*
      unit tests cover the movement state machine and it feels good by hand.
- [ ] **M1.6** Interaction: face a tile and press interact; objects dispatch by type.
      *Done when* a sign shows text.
- [ ] **M1.7** Doors and warps: fade transition, target map and spawn id, lazy-load
      interior maps and atlases on first entry. *Done when* entering and leaving a
      building keeps facing and position correct.
- [ ] **M1.8** Tap and click to move: A* on the collision grid (pure function, unit
      tested), path preview dots, tap on NPC or door walks adjacent then interacts.
      *Done when* it works on a real phone.
- [ ] **M1.9** Input layer: keyboard, gamepad, pointer unified into actions
      (`move`, `interact`, `back`, `menu`). *Done when* all three drive the same actions.
- [ ] **M1.10** Save system: versioned `localStorage` blob (position, map, stamps, flags,
...
- [ ] **M1.11** Deep links: `?at=<place-id>` spawns outside that place and skips the
      title. *Done when* every place id in the content map works.
- [ ] **M1.12** Greybox maps: overworld plus one interior, generated by a minimal
...
- [ ] **M1.13** Perf harness: an FPS and frame-time overlay behind `?debug`, plus a
