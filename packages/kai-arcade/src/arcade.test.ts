...
import { Board, COLS, FallingBlocks, fallDelay, linePoints, ROWS } from "./blocks.ts";
import { COLS as DCOLS, generateDungeon, nextStep } from "./dungeon.ts";
import { lifeStep } from "./life.ts";
import { NO_INPUT, rng, type ArcadeInput } from "./types.ts";
...
describe("every built-in game", () => {
	it("builds, runs and draws", async () => {
		const { BUILTIN_GAMES } = await import("./index.ts");
		...
		for (const [id, make] of Object.entries(BUILTIN_GAMES)) {
			const game = make({ best: () => 0, record: () => {} });
