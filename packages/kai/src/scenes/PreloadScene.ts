...
import { SERVICES_KEY, type GameServices } from "../boot.ts";
import { EMOTE_FRAME } from "../ui/emotes.ts";
import {
	ANIMS,
	DIRECTIONS,
	FRAME_HEIGHT,
	...
	type AnimName,
	type PortraitAnim,
} from "../characters/sheet.ts";
import { DialogueRunner } from "../dialogue/DialogueRunner.ts";
import { Progress, PROGRESS_KEY } from "../progress/Progress.ts";
import { browserStorage, loadSave, type SaveData } from "../save/save.ts";
...
export class PreloadScene extends Phaser.Scene {
	...
	preload() {
		...
		for (const [id, recipe] of Object.entries(services.data.content.characters)) {
	...
	create() {
		const services = this.registry.get(SERVICES_KEY) as GameServices;
		for (const id of Object.keys(services.data.content.characters)) {
		...
		const saved = loadSave(browserStorage(), services.config.saveKey);
