...
import { DECALS, DOOR, FENCE, PIER, PLATEAU } from "./palette.ts";
import { glow, NIGHT_LIGHTS } from "./lighting.ts";
import { PREFABS } from "./prefabs.ts";
import { single } from "./singles.ts";
import { seeded } from "@datagutt/kai-worldgen/random";
import type { MapCanvas, Prefab } from "@datagutt/kai-worldgen/canvas";
import { noise2, Region } from "@datagutt/kai-worldgen/layout";
 ...
 */
export function forest(c: MapCanvas, region: Region, seed: number, kinds: Prefab[], density = 0.6, taken?: Region) {
	...
	for (const [x, y] of cells) {
		...
		const prefab = kinds[Math.floor(rand() * kinds.length)];
...
export function building(
	...
	c.stamp(prefab, x, y);
	if (!prefab.door) throw new Error(`Prefab ${prefab.sheet} at (${prefab.col}, ${prefab.row}) has no door`);
