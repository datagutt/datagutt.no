...
- **Weather:** `lib/weather.ts` reads Oslo's current weather from MET Norway (Locationforecast; their terms require the identifying User-Agent) into the WorldState's `weather`. `game/world/weather.ts` turns it into the sky and the ambience; `game/fx/Weather.ts` draws it. `?debug&weather=<kind>` overrides it.
